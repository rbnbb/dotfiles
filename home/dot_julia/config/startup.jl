...
if Base.isinteractive() &&
   (local REPL = get(Base.loaded_modules, Base.PkgId(Base.UUID("3fa0cd96-eef1-5676-8a61-b3b8758bbffb"), "REPL"), nothing); REPL !== nothing)
    ...
    using OhMyREPL
    # using LiveServer  #look it up
    push!(LOAD_PATH, "/rbn/phy_study/phd/code/NoisyQubitMPS/src")
    ENV["JULIA_EXCLUSIVE"] = 1
    # ENV["PGFPLOTSX_PREAMBLE_PATH"] = "/home/rbn/.julia/config/pgfplotsx_custom_preamble.tex"
    # PLOTS_DEFAULTS = Dict(:fontfamily=>"Computer Modern")
    ...
    local tooling_dict = Dict{Symbol,Vector{Symbol}}(
        ...
        :TestEnv    => Symbol.(["TestEnv.activate()"]),
    )
    pushfirst!(REPL.repl_ast_transforms, function(ast::Union{Expr,Nothing})
